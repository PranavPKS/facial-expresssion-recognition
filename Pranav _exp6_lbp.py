...
while(cap.isOpened()):
    ...
    cv2.putText(faceROI,'.',(p2[0,0],p2[0,1]),cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,255),1)
    cv2.putText(faceROI,'.',(p2[1,0],p2[1,1]),cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,255),1)
    cv2.putText(faceROI,'.',(p2[2,0],p2[2,1]),cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,255),1)
    cv2.putText(faceROI,'.',(p2[3,0],p2[3,1]),cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,255),1)
    cv2.putText(faceROI,'.',(p2[4,0],p2[4,1]),cv2.FONT_HERSHEY_SIMPLEX, 1,(0,255,255),1)

    ##Get the hog features data by calling get_hog()
    lbp_data = get_lbp(ROI_gray,p2)
    ...
    dd = get_d(p2,avg_p3)   #get the differences between the points in the current expression and average neutral points
    ...
    #prob = np.float32(clf.item().predict_proba( lbp_data.reshape(1,-1) )[0])
