...
#tot_samp = np.vstack([samp1,samp2])
#tot_label = np.concatenate([label1,label2])
clf = np.load('clf_new.npy')
#clf = svm.SVC(kernel='linear', C=100,probability=True).fit(tot_samp, tot_label)

#clf = SGDClassifier(loss="hinge", penalty="l2", probability=True)
...
#x=[1,2,3,4,5,6]
bar_width = 0.35
rects = plt.bar(index,[0,0,0,0,0,0],bar_width)
plt.ion()
        ...
        try:
            ...
            if p2.size:
                ...
                try:
                    ...
                    hog_data = np.subtract(hog_data,avg_ini)
                    dd = get_d(p3,avg_p3)
                    hog_data = np.append(hog_data,dd)
                    ...
                    #print clf.item().predict_proba( np.subtract(hog_data,avg_ini).reshape(1,-1) )
